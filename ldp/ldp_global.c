...
ldp_global *ldp_global_create(mpls_instance_handle data)
{
  ...
  if (g) {
	...
	g->edge_inlabel = LDP_GLOBAL_DEF_EDGE_INLABEL;
  ...
  if (g) {
    ...
    while (nhp != NULL) {
      ...
      switch(nhp->info.type) {
        ...
        case MPLS_NH_IF:
		  ifname[0] = '\0';
		  mpls_ifmgr_get_name(g->ifmgr_handle, nhp->info.if_handle, ifname, sizeof(ifname));
          LDP_PRINT(g->user_data,"    IF %s handle %p", ifname, nhp->iff);
          break;
...
mpls_return_enum _ldp_global_add_inlabel(ldp_global * g, ldp_inlabel * i, ldp_fec *f)
{
  ldp_inlabel *ip = NULL;
...
#else
  result = mpls_mpls_insegment_add(g->mpls_handle, &i->info, &f->info);
#endif
...
ldp_if *ldp_global_find_if_handle(ldp_global * g, mpls_if_handle handle)
{
  ...
  if (g) {
    while (i != NULL) {
      if (mpls_ifmgr_compare(g->ifmgr_handle, i->handle, handle) == MPLS_SUCCESS)
        return i;
