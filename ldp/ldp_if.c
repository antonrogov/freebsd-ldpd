...
{
  LDP_PRINT(g->user_data, "if delete: %p\n", i);
...
mpls_return_enum ldp_if_startup(ldp_global * g, ldp_if * i)
{
  ...
  if (mpls_socket_multicast_if_join(g->socket_handle, g->hello_socket, i->handle,
...
mpls_return_enum ldp_if_shutdown(ldp_global * g, ldp_if * i)
{
  ...
  mpls_socket_multicast_if_drop(g->socket_handle, g->hello_socket, i->handle,
    &i->dest.addr);
