  ...
  uint32_t flag)
{
  ...
  if (flag & LDP_GLOBAL_CFG_EDGE_INLABEL) {
    g->edge_inlabel = global->edge_inlabel;
  }
#if MPLS_USE_LSR
  ...
  uint32_t flag)
{
  ...
  if (flag & LDP_GLOBAL_CFG_EDGE_INLABEL) {
    global->edge_inlabel = g->edge_inlabel;
  }
#if MPLS_USE_LSR
  if (flag & LDP_GLOBAL_CFG_LSR_HANDLE) {
    global->lsr_handle = g->lsr_handle ;
...
mpls_return_enum ldp_cfg_fec_change(mpls_cfg_handle handle, mpls_fec *f, mpls_nexthop *n, uint32_t flag)
{
	ldp_global *global = (ldp_global *) handle;
	ldp_fec *fec = NULL;
	ldp_nexthop *nh = NULL;
	...
	LDP_ENTER(global->user_data, "ldp_cfg_fec_change");
	mpls_lock_get(global->global_lock); /* LOCK */
	...
	if(flag & LDP_FEC_CFG_BY_INDEX)
		...
		fec = ldp_fec_find(global, f);
	if(!fec) {
		LDP_PRINT(global->user_data, "ldp_cfg_fec_change: no such fec\n");
		goto ldp_cfg_fec_change;
	}
	...
	if (flag & LDP_FEC_NEXTHOP_CFG_BY_INDEX)
		...
		nh = ldp_fec_nexthop_find(fec, n);
	if(!nh) {
		LDP_PRINT(global->user_data, "ldp_cfg_fec_change: no such nh\n");
		goto ldp_cfg_fec_change;
	}

	if(ldp_fec_process_change(global, fec, nh, nh, NULL) != MPLS_SUCCESS)
		MPLS_ASSERT(0);
