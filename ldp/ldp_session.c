...
static const char *AddressToString(struct in_addr addr)
{
	uint32_t a = (uint32_t)addr.s_addr;
	snprintf(addrBuf, sizeof(addrBuf), "%u.%u.%u.%u", a & 0xff, (a >> 8) & 0xff, (a >> 16) & 0xff, (a >> 24) & 0xff);
	return addrBuf;
}
void _ldp_session_add_adj(ldp_session * s, ldp_adj * a)
{
  ...
  sprintf(s->session_name, "%s:%d", AddressToString(lsr_address),
