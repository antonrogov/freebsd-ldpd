...
int Mpls_encodeLdpFecAdrEl
  ...
  switch (type) {
    ...
    case MPLS_PWID_FEC:
      {
        if (MPLS_FEC_PWIDLEN > bufSize) {
          return MPLS_ENC_BUFFTOOSMALL;
        }
		
        /*
        *tempBuf = fecAdrEl->pwidEl.type;
        tempBuf++;

        pw_type = fecAdrEl->pwidEl.pw_type | (fecAdrEl->pwidEl.has_control_word << 15);
        MEM_COPY(tempBuf, (u_char *) & (pw_type), 2);
        temp_buf += 2;

		*tempBuf = 4;
        tempBuf++;

        MEM_COPY(tempBuf, (u_char *) & (fecAdrEl->pwidEl.group_id), 4);
        temp_buf += 4;

        MEM_COPY(tempBuf, (u_char *) & (fecAdrEl->pwidEl.pw_id), 4);
        temp_buf += 4;
        */
        MEM_COPY(tempBuf, (u_char *) & (fecAdrEl->pwidEl), MPLS_FEC_PWIDLEN);
        encodedSize = MPLS_FEC_PWIDLEN;
        break;
      }
    default:
      {
    ...
    case MPLS_PWID_FEC:
      {
        ...
        MEM_COPY((u_char *) & (fecAdrEl->pwidEl), tempBuff, MPLS_FEC_PWIDLEN);
        decodedSize = MPLS_FEC_PWIDLEN;
        break;
      }
    default:
      {
